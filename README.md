...
[_`Fork`_][fork] this repository to add features, once tested and the `_contributers` directory has been updated issue a [_`Pull Request`_][pull-request] to have your changes reviewed and possibly merged into the code base.
...
- [_`make_scriptlets/`_][source_make_scriptlets_dir] contains scripts used by [_`Makefile`_][source_makefile] for [_`make install`_][docs_install] and [_`make update`_][docs_update] commands; use `make list` to list all available `make` commands
...
- [_`jekyll_wwwconf.sh`_][source_jekyll-wwwconf], adds {sub-}domain names to web server for given user/repo, currently _wired_ for Nginx Web Server, check the published [_`usage examples`_][docs_jekyll_wwwconf] for
